...
def loadData(fileName):
    ...
    #返回data和label
    return dataList[:200], labelList[:200]
...
class maxEnt:
            ...
            for feature in range(self.featureNum):
                for y in range(2):
                    if (self.trainDataList[i][feature], y) in self.fixy[feature]:
                        ...
                        Epxy[id] += (1 / self.N) * Pwxy[y] # 这里的1/self.N是表示先验值p(x)
        return Epxy
    ...
    def calc_fixy(self):
        ...
        fixyDict = [defaultdict(int) for i in range(self.featureNum)] #featureNum特征大小
        #遍历训练集中所有样本
...
if __name__ == '__main__':
    ...
    #初始化最大熵类
    maxEnt = maxEnt(trainData, trainLabel, testData, testLabel)
